...
import type { ConduitControllerInterface, ConsiderationInterface } from "../typechain-types";
...
describe(`Getter tests (Seaport v${VERSION})`, function () {
  ...
  before(async () => {
    ...
    ({ conduitController, directMarketplaceContract, marketplaceContract } = await seaportFixture(owner));
  });
  ...
  it("gets correct version, domain separator and conduit controller", async () => {
    ...
    const { version, domainSeparator, conduitController: controller } = await marketplaceContract.information();
    ...
    expect(domainSeparator).to.equal(
      keccak256(`0x${typehash.slice(2)}${namehash.slice(2)}${versionhash.slice(2)}${chainIdEncoded}${addressEncoded}`)
    );
