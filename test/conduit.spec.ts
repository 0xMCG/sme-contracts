...
import { getItemETH, random128, randomBN, randomHex, toAddress, toBN, toFulfillment } from "./utils/encoding";
import { faucet } from "./utils/faucet";
import { fixtureERC1155, fixtureERC20, fixtureERC721, seaportFixture } from "./utils/fixtures";
import { VERSION, getCustomRevertSelector, minRandom, simulateMatchOrders } from "./utils/helpers";
...
describe(`Conduit tests (Seaport v${VERSION})`, function () {
  ...
  it("Adds a channel, and executes transfers (ERC1155 with batch)", async () => {
    ...
    await conduitController.connect(owner).updateChannel(tempConduit.address, seller.address, true);
  ...
  it("Adds a channel, and executes transfers (ERC721)", async () => {
    ...
    await expect(testERC721.connect(seller).setApprovalForAll(tempConduit.address, true))
  ...
  it("Adds a channel, and executes transfers (ERC721 + ERC20)", async () => {
    ...
    await expect(testERC20.connect(seller).approve(tempConduit.address, tokenAmount))
    ...
    expect(await testERC20.balanceOf(buyer.address)).to.equal(tokenAmount.sub(100));
  });
  ...
  it("Adds a channel, and executes transfers (ERC721 + ERC1155)", async () => {
    ...
    await expect(testERC1155.connect(seller).setApprovalForAll(tempConduit.address, true))
      .to.emit(testERC1155, "ApprovalForAll")
      .withArgs(seller.address, tempConduit.address, true);

    // Check ownership
    expect(await testERC1155.balanceOf(seller.address, secondNftId)).to.equal(amount);
    ...
    expect(await testERC721.ownerOf(nftId)).to.equal(buyer.address);
    // Check balance
    expect(await testERC1155.balanceOf(seller.address, secondNftId)).to.equal(10);
    expect(await testERC1155.balanceOf(buyer.address, secondNftId)).to.equal(amount.sub(10));
  });
  ...
  it("Adds a channel, and executes transfers (ERC20 + ERC1155)", async () => {
    ...
    // Check ownership
    expect(await testERC1155.balanceOf(seller.address, nftId)).to.equal(erc1155amount);
    ...
    expect(await testERC1155.balanceOf(buyer.address, nftId)).to.equal(erc1155amount.sub(10));
  });
  ...
  it("Adds a channel, and executes transfers (ERC20 + ERC721 + ERC1155)", async () => {
    ...
    const erc20Transfer = await createTransferWithApproval(testERC20, seller, 1, tempConduit.address, seller.address, buyer.address);

    // Create/Approve Y amount of  ERC721s
    const erc721Transfer = await createTransferWithApproval(testERC721, seller, 2, tempConduit.address, seller.address, buyer.address);

    // Create/Approve Z amount of ERC1155s
    const erc1155Transfer = await createTransferWithApproval(testERC1155, seller, 3, tempConduit.address, seller.address, buyer.address);

    // Send an ERC20, ERC721, and ERC1155
    await tempConduit.connect(seller).execute([erc20Transfer, erc721Transfer, erc1155Transfer]);

    // Check ownership
    expect(await testERC721.ownerOf(erc721Transfer.identifier)).to.equal(buyer.address);
    // Check balance
    expect(await testERC20.balanceOf(seller.address)).to.equal(0);
    expect(await testERC20.balanceOf(buyer.address)).to.equal(erc20Transfer.amount);
    expect(await testERC1155.balanceOf(seller.address, erc1155Transfer.identifier)).to.equal(0);
    expect(await testERC1155.balanceOf(buyer.address, erc1155Transfer.identifier)).to.equal(erc1155Transfer.amount);
  });
  ...
  it("Adds a channel, and executes transfers (many token types)", async () => {
    ...
    const transfers = [...erc20Transfers, ...erc721Transfers, ...erc1155Transfers];
    const contracts = [...erc20Contracts, ...erc721Contracts, ...erc1155Contracts];
    // Send the transfers
    ...
    for (let i = 0; i < transfers.length; i++) {
      ...
      switch (itemType) {
        case 1: // ERC20
          // Check balance
          expect(await (token as typeof erc20Contracts[0]).balanceOf(from)).to.equal(0);
          expect(await (token as typeof erc20Contracts[0]).balanceOf(to)).to.equal(amount);
          break;
        case 2: // ERC721
        case 4: // ERC721_WITH_CRITERIA
          expect(await (token as typeof erc721Contracts[0]).ownerOf(identifier)).to.equal(to);
          break;
        case 3: // ERC1155
  ...
  it("Reverts on calls to batch transfer 1155 items with no contract on a conduit", async () => {
    await conduitController.connect(owner).updateChannel(tempConduit.address, owner.address, true);
  ...
  if (!process.env.REFERENCE) {
    ...
    it("ERC1155 batch transfer reverts with generic error if it has insufficient gas to copy revert data", async () => {
      ...
      await expect(
        ...
        .to.be.revertedWithCustomError(tempConduit, "ERC1155BatchTransferGenericFailure")
  ...
  it("Makes batch transfer 1155 items through a conduit", async () => {
    ...
    const { conduit: tempConduitAddress } = await conduitController.getConduit(tempConduitKey);

    await conduitController.connect(owner).createConduit(tempConduitKey, owner.address);
      ...
      [
        {
          ...
          from: owner.address,
          to: buyer.address,
          ids: [nftId, secondNftId, thirdNftId, nftId4, nftId5, nftId6, nftId7, nftId8, nftId9, nftId10],
          amounts: [amount, secondAmount, thirdAmount, amount4, amount5, amount6, amount7, amount8, amount9, amount10],
        },
      ]
    );
  });
  ...
  it("Performs complex batch transfer through a conduit", async () => {
    ...
    await expect(testERC1155Two.connect(owner).setApprovalForAll(tempConduit.address, true))
    ...
    expect(await testERC1155.balanceOf(newAddress, secondNftId)).to.equal(secondAmount);
    expect(await testERC1155.balanceOf(newAddress, thirdNftId)).to.equal(thirdAmount);
    expect(await testERC1155.balanceOf(newAddress, nftId4)).to.equal(amount4);

    expect(await testERC1155Two.balanceOf(newAddress, nftId5)).to.equal(amount5);
    expect(await testERC1155Two.balanceOf(newAddress, nftId6)).to.equal(amount6);
    expect(await testERC1155Two.balanceOf(newAddress, nftId7)).to.equal(amount7);
    expect(await testERC1155Two.balanceOf(newAddress, nftId8)).to.equal(amount8);
  ...
  it("Reverts when attempting to update a conduit channel when call is not from controller", async () => {
    await expect(conduitOne.connect(owner).updateChannel(ethers.constants.AddressZero, true)).to.be.revertedWithCustomError(
      conduitOne,
      "InvalidController"
    );
  });
  ...
  it("Reverts when attempting to execute transfers on a conduit when not called from a channel", async () => {
    const expectedRevertReason = getCustomRevertSelector("ChannelClosed(address)") + owner.address.slice(2).padStart(64, "0").toLowerCase();
  ...
  it("Reverts when attempting to execute with 1155 transfers on a conduit when not called from a channel", async () => {
    await expect(conduitOne.connect(owner).executeWithBatch1155([], [])).to.be.revertedWithCustomError(conduitOne, "ChannelClosed");
  });

  it("Reverts when attempting to execute batch 1155 transfers on a conduit when not called from a channel", async () => {
    await expect(conduitOne.connect(owner).executeBatch1155([])).to.be.revertedWithCustomError(conduitOne, "ChannelClosed");
  });
  ...
  it("Retrieves the owner of a conduit", async () => {
    ...
    await expect(conduitController.connect(owner).ownerOf(buyer.address)).to.be.revertedWithCustomError(conduitController, "NoConduit");
  });
  ...
  it("Retrieves the key of a conduit", async () => {
    ...
    await expect(conduitController.connect(owner).getKey(buyer.address)).to.be.revertedWithCustomError(conduitController, "NoConduit");
  });

  it("Retrieves the status of a conduit channel", async () => {
    let isOpen = await conduitController.getChannelStatus(conduitOne.address, marketplaceContract.address);
    expect(isOpen).to.be.true;

    isOpen = await conduitController.getChannelStatus(conduitOne.address, seller.address);
    expect(isOpen).to.be.false;

    await expect(conduitController.connect(owner).getChannelStatus(buyer.address, seller.address)).to.be.revertedWithCustomError(
      conduitController,
      "NoConduit"
    );
  });
  ...
  it("Retrieves conduit channels from the controller", async () => {
    const totalChannels = await conduitController.getTotalChannels(conduitOne.address);
    expect(totalChannels).to.equal(1);

    await expect(conduitController.connect(owner).getTotalChannels(buyer.address)).to.be.revertedWithCustomError(
      conduitController,
      "NoConduit"
    );

    const firstChannel = await conduitController.getChannel(conduitOne.address, 0);
    expect(firstChannel).to.equal(marketplaceContract.address);

    await expect(conduitController.connect(owner).getChannel(buyer.address, +totalChannels - 1)).to.be.revertedWithCustomError(
      conduitController,
      "NoConduit"
    );

    await expect(conduitController.connect(owner).getChannel(conduitOne.address, 1)).to.be.revertedWithCustomError(
      conduitController,
      "ChannelOutOfRange"
    );

    await expect(conduitController.connect(owner).getChannel(conduitOne.address, 2)).to.be.revertedWithCustomError(
      conduitController,
      "ChannelOutOfRange"
    );
    ...
    await expect(conduitController.connect(owner).getChannels(buyer.address)).to.be.revertedWithCustomError(conduitController, "NoConduit");
  });

  it("Adds and removes channels", async () => {
    // Get number of open channels
    let totalChannels = await conduitController.getTotalChannels(conduitOne.address);
    expect(totalChannels).to.equal(1);
    ...
    // No-op
    await expect(conduitController.connect(owner).updateChannel(conduitOne.address, marketplaceContract.address, true)).to.be.reverted; // ChannelStatusAlreadySet

    isOpen = await conduitController.getChannelStatus(conduitOne.address, marketplaceContract.address);
    expect(isOpen).to.be.true;

    // Get number of open channels
    totalChannels = await conduitController.getTotalChannels(conduitOne.address);
    expect(totalChannels).to.equal(1);

    await conduitController.connect(owner).updateChannel(conduitOne.address, seller.address, true);
    ...
    await conduitController.connect(owner).updateChannel(conduitOne.address, marketplaceContract.address, false);
    ...
    const conduitControllerSigner = await ethers.getSigner(conduitController.address);

    await conduitOne.connect(conduitControllerSigner).updateChannel(marketplaceContract.address, true);
    ...
    await conduitController.connect(owner).updateChannel(conduitOne.address, seller.address, false);
    ...
    await conduitController.connect(owner).updateChannel(conduitOne.address, marketplaceContract.address, true);
  ...
  it("Reverts on an attempt to move an unsupported item", async () => {
    ...
    const isOpen = await conduitController.getChannelStatus(conduitOne.address, seller.address);
    expect(isOpen).to.be.true;
  ...
  it("Reverts when attempting to create a conduit not scoped to the creator", async () => {
    await expect(conduitController.connect(owner).createConduit(ethers.constants.HashZero, owner.address)).to.be.revertedWithCustomError(
      conduitController,
      "InvalidCreator"
    );
  });
  ...
  it("Reverts when attempting to create a conduit that already exists", async () => {
    await expect(conduitController.connect(owner).createConduit(conduitKeyOne, owner.address))
  ...
  it("Reverts when attempting to update a channel for an unowned conduit", async () => {
    await expect(conduitController.connect(buyer).updateChannel(conduitOne.address, buyer.address, true))
  ...
  it("Retrieves no initial potential owner for new conduit", async () => {
    const potentialOwner = await conduitController.getPotentialOwner(conduitOne.address);
    expect(potentialOwner).to.equal(ethers.constants.AddressZero);

    await expect(conduitController.connect(owner).getPotentialOwner(buyer.address)).to.be.revertedWithCustomError(
      conduitController,
      "NoConduit"
    );
  });
  ...
  it("Lets the owner transfer ownership via a two-stage process", async () => {
    await expect(conduitController.connect(buyer).transferOwnership(conduitOne.address, buyer.address)).to.be.revertedWithCustomError(
      conduitController,
      "CallerIsNotOwner"
    );

    await expect(
      conduitController.connect(owner).transferOwnership(conduitOne.address, ethers.constants.AddressZero)
    ).to.be.revertedWithCustomError(conduitController, "NewPotentialOwnerIsZeroAddress");

    await expect(conduitController.connect(owner).transferOwnership(seller.address, buyer.address)).to.be.revertedWithCustomError(
      conduitController,
      "NoConduit"
    );

    let potentialOwner = await conduitController.getPotentialOwner(conduitOne.address);
    expect(potentialOwner).to.equal(ethers.constants.AddressZero);

    await conduitController.transferOwnership(conduitOne.address, buyer.address);

    potentialOwner = await conduitController.getPotentialOwner(conduitOne.address);
    expect(potentialOwner).to.equal(buyer.address);

    await expect(conduitController.connect(owner).transferOwnership(conduitOne.address, buyer.address)).to.be.revertedWithCustomError(
      conduitController,
      "NewPotentialOwnerAlreadySet"
    );

    await expect(conduitController.connect(buyer).cancelOwnershipTransfer(conduitOne.address)).to.be.revertedWithCustomError(
      conduitController,
      "CallerIsNotOwner"
    );

    await expect(conduitController.connect(owner).cancelOwnershipTransfer(seller.address)).to.be.revertedWithCustomError(
      conduitController,
      "NoConduit"
    );
    ...
    await expect(conduitController.connect(owner).cancelOwnershipTransfer(conduitOne.address)).to.be.revertedWithCustomError(
      conduitController,
    ...
    await expect(conduitController.connect(buyer).acceptOwnership(seller.address)).to.be.revertedWithCustomError(
      conduitController,
      "NoConduit"
    );

    await expect(conduitController.connect(seller).acceptOwnership(conduitOne.address)).to.be.revertedWithCustomError(
      conduitController,
