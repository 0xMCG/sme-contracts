...
import type { AdvancedOrder, ConsiderationItem, CriteriaResolver, OfferItem } from "../types";
import type { BigNumber, BigNumberish, ContractReceipt, ContractTransaction, Wallet } from "ethers";

export { fixtureERC20, fixtureERC721, fixtureERC1155, tokensFixture } from "./tokens";

const { provider } = ethers;

export const marketFixture = async (owner: Wallet) => {
  ...
  const withBalanceChecks = async (
    ...
    for (const offeredItem of allOfferedItems as any[]) {
      ...
      if (offeredItem.itemType === 0) {
        ...
        // ERC1155
        offeredItem.initialBalance = await tokenByType[offeredItem.itemType].balanceOf(
          offeredItem.account,
      ...
      if (offeredItem.itemType === 2) {
        ...
        offeredItem.ownsItemBefore =
          (await tokenByType[offeredItem.itemType].ownerOf(offeredItem.identifierOrCriteria)) === offeredItem.account;
      }
    }
    ...
    for (const receivedItem of allReceivedItems as any[]) {
      ...
      if (receivedItem.itemType === 0) {
        ...
        // ERC1155
        receivedItem.initialBalance = await tokenByType[receivedItem.itemType].balanceOf(
      ...
      if (receivedItem.itemType === 2) {
        ...
        receivedItem.ownsItemBefore =
          (await tokenByType[receivedItem.itemType].ownerOf(receivedItem.identifierOrCriteria)) === receivedItem.recipient;
      }
    }
    ...
    for (const receivedItem of allReceivedItems as any[]) {
      ...
      if (receivedItem.itemType === 0) {
        ...
        // ERC1155
        receivedItem.finalBalance = await tokenByType[receivedItem.itemType].balanceOf(
          receivedItem.recipient,
          receivedItem.identifierOrCriteria
        );
      } else {
  ...
  const checkTransferEvent = async (
    ...
    { offerer, target }: { offerer: string; target: string }
  ) => {
    ...
    if ([1, 2, 5].includes(itemType)) {
      const contract = new Contract(token, (itemType === 1 ? testERC20 : testERC721).interface, provider);
      await expect(tx)
  ...
  const checkExpectedEvents = async (
    ...
    if (standardExecutions && standardExecutions.length) {
      for (const standardExecution of standardExecutions) {
        const { item, offerer } = standardExecution;
    ...
    for (let { order, orderHash, fulfiller, fulfillerConduitKey, recipient } of orderGroups) {
      ...
      if (txMethod in matchMethods) {
        ...
        expect(marketplaceContractOrdersMatchedEvents[0].orderHashes.length).to.equal(orderGroups.length);
      ...
      const compareEventItems = async (item: any, orderItem: OfferItem | ConsiderationItem, isConsiderationItem: boolean) => {
        ...
        if (order.parameters.orderType === 0) {
          ...
          if (orderItem.startAmount.toString() === orderItem.endAmount.toString()) {
            expect(item.amount.toString()).to.equal(orderItem.endAmount.mul(order.numerator).div(order.denominator).toString());
          } else {
      ...
      if (!standardExecutions || !standardExecutions.length) {
        ...
        for (const item of offer) {
          ...
          let amount;
          if (order.parameters.orderType === 0) {
            amount = startAmount.eq(endAmount) ? endAmount : startAmount.mul(remaining).add(endAmount.mul(elapsed)).div(duration);
          } else {
            amount = endAmount.mul(order.numerator).div(order.denominator);
          }
          amount = amount.mul(multiplier);
      ...
      for (const [index, offer] of Object.entries(event.offer) as any[]) {
        ...
        if (offer.itemType === 1 && standardExecutions.map((x) => x.item.itemType).includes(1)) {
          // ERC20
          // search for transfer
          const transferLogs = (tokenEvents ?? [])
            .map((x) => testERC20.interface.parseLog(x))
            .filter(
              (x) => x.signature === "Transfer(address,address,uint256)" && x.args.from === event.offerer /* &&
          ...
          // }
        } else if (offer.itemType === 2 && standardExecutions.map((x) => x.item.itemType).includes(2)) {
          // ERC721
          // search for transfer
          const transferLogs = (tokenEvents ?? [])
        ...
        } else if (offer.itemType === 3 && standardExecutions.map((x) => x.item.itemType).includes(3)) {
          // search for transfer
          const transferLogs = (tokenEvents ?? [])
            .map((x) => testERC1155.interface.parseLog(x))
            .filter(
              (x) =>
                (x.signature === "TransferSingle(address,address,address,uint256,uint256)" && x.args.from === event.offerer) /* &&
                  // TODO: work out better way to check recipient with new matchOrder logic
                  (fulfiller !== constants.AddressZero
                    ? x.args.to === fulfiller
                    : true) */ ||
                (x.signature === "TransferBatch(address,address,address,uint256[],uint256[])" && x.args.from === event.offerer) /* &&
                  // TODO: work out better way to check recipient with new matchOrder logic
                  (fulfiller !== constants.AddressZero
                    ? x.args.to === fulfiller
                    : true) */
          ...
          for (const transferLog of transferLogs) {
            if (
              ...
              (shouldSkipAmountComparison || transferLog.args.amount.toString() === offer.amount.mul(multiplier).toString())
            ) {
              found = true;
              break;
            }
      ...
      for (const [index, consideration] of Object.entries(event.consideration) as any[]) {
        ...
        if (consideration.itemType === 1 && standardExecutions.map((x) => x.item.itemType).includes(1)) {
          // ERC20
          // search for transfer
          const transferLogs = (tokenEvents ?? [])
            .map((x) => testERC20.interface.parseLog(x))
            .filter((x) => x.signature === "Transfer(address,address,uint256)" && x.args.to === consideration.recipient);

          expect(transferLogs.length).to.be.above(0);
          // TODO: check each transferred amount
          // for (const transferLog of transferLogs) {
          // }
        } else if (consideration.itemType === 2 && standardExecutions.map((x) => x.item.itemType).includes(2)) {
          // ERC721
          // search for transfer
          const transferLogs = (tokenEvents ?? [])
            .map((x) => testERC721.interface.parseLog(x))
        ...
        } else if (consideration.itemType === 3 && standardExecutions.map((x) => x.item.itemType).includes(3)) {
          // search for transfer
          const transferLogs = (tokenEvents ?? [])
            .map((x) => testERC1155.interface.parseLog(x))
            .filter(
              (x) =>
                (x.signature === "TransferSingle(address,address,address,uint256,uint256)" && x.args.to === consideration.recipient) ||
                (x.signature === "TransferBatch(address,address,address,uint256[],uint256[])" && x.args.to === consideration.recipient)
            );
          ...
          for (const transferLog of transferLogs) {
            if (
              ...
              (shouldSkipAmountComparison || transferLog.args.amount.toString() === consideration.amount.mul(multiplier).toString())
            ) {
              found = true;
              break;
            }
...
export type MarketFixtures = Awaited<ReturnType<typeof marketFixture>>;
